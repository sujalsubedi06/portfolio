        ...
        <div className="mt-10 grid grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-12">
          ...
          <RevealItem index={1} direction="up" className="lg:col-span-3">
            <Card className="flex h-full flex-col">
              ...
              <ul className="mt-3 flex-1 space-y-3" role="list">
                {CERTIFICATIONS.map((cert) => (
                  <li key={cert.name} className="flex items-baseline justify-between gap-2">
                    <span>
            ...
            <Card className="h-full">
              ...
              <ul className="mt-3 space-y-2.5" role="list">
                {CURRENTLY_LEARNING.map((item) => (
                  ...
                  >
                    <span
                      className="h-1.5 w-1.5 shrink-0 rounded-full bg-[var(--color-purple)]"
                      aria-hidden="true"
                    />
                    {item}
                  </li>
          ...
          <RevealItem index={3} direction="right" className="lg:col-span-4">
            <Card className="h-full">
              <div className="flex items-center justify-between">
                ...
                <ArrowLink href="https://github.com/sujal-subedi" size="sm">
                  View profile
                </ArrowLink>
              </div>
              <h3 className="mt-4 text-sm font-semibold text-[var(--color-text)]">
                ...
                {GITHUB_STATS.map((stat, i) => (
                  <div key={stat.label}>
                    ...
                    <dd className="mt-0.5 text-base font-semibold text-[var(--color-text)]">
                      <CountUp value={stat.value} delay={0.15 * i} duration={1.1} />
                    </dd>
                  </div>
                ))}
              </dl>
